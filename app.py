    ...
    if st.session_state.scan_complete and os.path.exists(st.session_state.final_map_path):
        ...
        with st.spinner("Rendering web-optimized high-res preview..."):
            ...
            if view_mode == "Side-by-Side Compare":
                ...
                sub_col1.image(orig_img, caption="Raw Drone Telemetry", use_container_width=True)
                sub_col2.image(viz_img, caption=f"AI Extraction: {class_choice}", use_container_width=True)
                ...
                st.image(overlay, caption="X-Ray Deep Blend", use_container_width=True)
